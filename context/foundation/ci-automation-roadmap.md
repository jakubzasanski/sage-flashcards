...
| S-03 | cd-migrate-and-deploy    | merges to master ship Supabase migrations → Cloudflare Worker                     | S-01, F-01, (S-02) | #2           | done     |
...
- **CD:** ✅ present (S-03 done, 2026-06-27) — `deploy.yml` ships migrations + Worker on green `master` (workflow_run-gated); first live auto-deploy verified.
...
- **As shipped (2026-06-27, PR #26 squashed to `3dff7ac`):** `deploy.yml` gated via `workflow_run` on **CI** success for a `master` `push` (drops PRs/red runs), checks out the tested `head_sha`, then **build → `supabase db push` (dry-run logged first) → `wrangler secret bulk` → `wrangler deploy`**, scoped to a `production` environment. Build precedes migrate so a build failure aborts before any DB mutation. `wrangler.jsonc` declares the `SESSION` KV (`14209e17…`) + `IMAGES` bindings for non-interactive deploy (the original blocking unknown). Failure policy: fail-loud, no auto-rollback (`wrangler rollback` for the Worker, Supabase PITR for the DB). Runbook: `context/deployment/deploy-plan.md` → "Continuous deployment (S-03)". First live auto-deploy verified end-to-end (live version `a9d2c025`); the fail-loud path was exercised for real (first run died on a malformed `CLOUDFLARE_API_TOKEN`, deploy skipped, prod untouched).
- **Status:** done
...
| S-03       | cd-migrate-and-deploy    | CD: migrate Supabase → deploy Worker on green master   | done                  | ✅ shipped 2026-06-27 (PR #26) |
...
- **S-03: CD — Supabase migrations → Cloudflare Worker on green master** — Shipped 2026-06-27 via PR #26 (squashed to `3dff7ac`). `deploy.yml` is `workflow_run`-gated on **CI** success for a `master` `push`, checks out the tested `head_sha`, then runs **build → `supabase db push` (dry-run logged) → `wrangler secret bulk` → `wrangler deploy`** in a `production` environment; `wrangler.jsonc` declares `SESSION` KV + `IMAGES` for non-interactive deploy. First live auto-deploy verified end-to-end (live version `a9d2c025`, smoke 200/200/200/302). Lessons: (1) fail-loud worked for real — the first run died on a malformed `CLOUDFLARE_API_TOKEN` (`code 6111 Invalid format for Authorization header`, usually a stray newline from `gh secret set`); set tokens with `printf %s` to avoid the trailing newline; (2) migrate ran before the failed secret step, so migrations must be idempotent/re-runnable for a clean `gh run rerun` — they were; (3) build-before-migrate ordering meant a hypothetical build break would have aborted before any DB mutation.
