// Local-only E2E configuration.
//
// The keys below are the well-known Supabase **local development** demo keys — identical on
// every `npx supabase start`. They are used solely to seed and clean the local test database,
// never production secrets, and never to assert RLS isolation (per lessons.md, asserting
...
// A dedicated, confirmed E2E user. `enable_confirmations = false` in supabase/config.toml,
